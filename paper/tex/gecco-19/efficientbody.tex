...
particles being initialized close to the optimum. However, moving to a higher dimensional
...
and from $U((x_{\min},\lambda x_{\min})\cup(\lambda x_{\max},
x_{\max}))$. Despite this result, we initialize our particles in the outer
quarter of the space (i.e., $U(.75x_{\max}, x_{\max})\cup U(x_{\min}, .75x_{\min})$)
...
performs communication between independent processes. Within the OpenMP and MPI
...
kernel\footnote{A kernel in GPGPU programming is the name of the procedure being
  run on  the GPU} is allowed, if thread branches diverge (i.e., threads take different
...
kernel. When grouped threads (referred to as a warp) within a GPU
...
GPU-based algorithm. Improvements to the CPU and GPU PSO algorithms directly
